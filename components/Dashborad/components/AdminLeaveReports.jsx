...
import { AdminleaveApprovalColumns } from "@/components/Dashborad/components/tableColumn";
...
const AdminLeaveReports = () => {
  const { handleAdminMonthlyReport, handleSendReport } = useUserProvider();
  ...
  useEffect(() => {
    async function fetchData() {
      const data = await handleAdminMonthlyReport(date);
      console.log(data);
      setTableData(data);
    }
    fetchData();
  }, [date, handleAdminMonthlyReport]);
      ...
      <CardContent className="pl-2">
        <Suspense fallback={<>Loading...</>}>
          <UserTable
            ...
            columns={AdminleaveApprovalColumns}
            limitNo={10}
          />
        </Suspense>
      </CardContent>
